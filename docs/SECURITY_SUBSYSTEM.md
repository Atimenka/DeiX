...
> **ПРИМЕЧАНИЕ ДЛЯ РАЗРАБОТЧИКОВ / LLM**: Этот документ является ранним концептуальным предложением и исторической спецификацией подсистемы безопасности. Текущая актуальная реализация находится непосредственно в коде репозитория (`src/`).
