...
pub fn back_to_text() {
    ...
    // Шрифт грузим ПОСЛЕ restore_text_mode(): тот перепрограммирует
    // Sequencer и Graphics Controller, а вместе с ними сбрасывает
    // доступ к плоскости 2, где лежит знакогенератор. Загрузка до
    // сброса просто теряется — на экране остаются полосы.
    crate::font::install_cyrillic_font();

    // После графики в видеопамяти лежит мусор от framebuffer'а.
    // Текстовый буфер 0xB8000 (80x25, 2 байта на символ) заполняем
    // пробелами с обычным атрибутом, иначе на экране остаётся каша.
    unsafe {
    ...
    // Графический режим затирает знакогенератор VGA (плоскость 2), где
    // лежит шрифт. Без перезагрузки текстовый экран превращается в
    // вертикальные полосы — символы рисуются мусорными глифами.
